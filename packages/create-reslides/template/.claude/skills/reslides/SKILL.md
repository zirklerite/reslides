...
| `Code` | Themed code block (no syntax highlighting) |
...
| Component | Props |
|---|---|
| `Text` | `size?: 'sm' \| 'md' \| 'lg' \| 'xl'` (default `'md'`), `muted?: boolean` (default `false`), `align?: 'left' \| 'center' \| 'right'` (default `'left'`), `children: Snippet` |
| `Image` | `src: string`, `alt?: string`, `fit?: 'cover' \| 'contain' \| 'fill'` (default `'contain'`), `caption?: string` |
| `Code` | `code: string`, `lang?: string` |
| `List` | `ordered?: boolean` (default `false`), `children: Snippet` |
| `Background` | `color?: string` (CSS color), `gradient?: string` (CSS gradient), `children?: Snippet`. Always fills slide at z-index 0. Place in Deck for global background, in Slide for per-slide override. |
